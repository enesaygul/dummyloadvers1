...
int temp = 0;    // temperature in integer format
char temp_char[3];  // temperature in character format to write in the file
...
const float k_voltageReference = 2.5;
...
char const *language_voltage = "V";
char const *language_current = "I";
char const *language_CutOff = "Cutoff Voltage";
char const *language_power = "P";
...
float readAdc(int channel) {
  ...
  // 2.048V.
  float value = (float(digitalValue) * 2.55) / 4096.000;
...
void readInputVoltage() {
  g_inputVoltage = (readAdc(k_adcInputVoltage)) * 12.45;
...
void setLoadCurrent(int setMode) {
  ...
  if (g_lcdDisplay != k_displayMenu) {
    ...
    case 0:
     ...
     // Resistance Mode
      g_setResistance = g_encoderPos / 50.000;
      if (g_setResistance < 0)
        g_setResistance = 0;

       EEPROM.put(address, g_setResistance );
       Cutoff_V = EEPROM.get(address, g_setResistance);
     ...
     /* if (g_setCurrent > k_maximumCurrent) {
        ...
        g_encoderPos = int(g_setResistance * 50.00);
      }
   ...
   // Current Mode
      g_setCurrent = g_encoderPos / 20.000; // as the DAC is capable of increasing the current in 0.002A
      if (g_setCurrent < 0)
        g_setCurrent = 0;
     // if (g_setCurrent > k_maximumCurrent)
       // g_setCurrent = k_maximumCurrent;

if(g_setCurrent = g_encoderPos / 20.000) {         
          incTime();
          setsecond = (hours*3600) + (minutes*60) + seconds ;
          setAh = g_setCurrent*(setsecond/3600);
          setWh = setAh * g_inputVoltage ;

}
   
if(g_inputVoltage <= Cutoff_V){
 g_setCurrent = 0;
  hours = 0 ;
  minutes = 0 ;
  seconds = 0 ;
    ...
    case 2:
      // Power Mode
      g_setPower = g_encoderPos / 50.000;
      if (g_setPower < 0)
        g_setPower = 0;
      if (g_setPower > k_maximumPower)
      ...
      if (g_setCurrent > k_maximumCurrent) {
        ...
        g_encoderPos = int(g_setPower * 50.00);
      }
...
void updateLCD(int displayType) {
  ...
  case 0:
    if (updateDisplay()) {
      ...
      lcd.print(buf);
      lcd.setCursor(2, 0);
      lcd.print(g_inputVoltage, 1);
      lcd.setCursor(0, 1);
      sprintf(buf, "%s=", language_current);
      lcd.print(buf);
      lcd.setCursor(2, 1);
      lcd.print(g_measuredCurrent, 1);
...
// Start loop function:
void loop() {
      digitalWrite(LEDTEST, HIGH);   // turn the LED on (HIGH is the voltage level)
  delay(1000);                       // wait for a second
  digitalWrite(LEDTEST, LOW);    // turn the LED off by making the voltage LOW
  delay(1000);    
